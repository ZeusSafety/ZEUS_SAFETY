...
DROP PROCEDURE IF EXISTS sp_Dashboard_ZeusElectric$$

CREATE PROCEDURE sp_Dashboard_ZeusElectric(
...
BEGIN
  ...
  SET @where_clause = CONCAT('vo.FECHA BETWEEN ''', v_inicio, ''' AND ''', v_fin, ''' AND (vo.LINEA = ''ZEUS-ELECTRIC'' OR vo.LINEA = ''ZEUS ELECTRIC'')');
