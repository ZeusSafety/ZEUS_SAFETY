...
export default function AsistenciasPage() {
  ...
  const formatMinutesToTime = (totalMinutes) => {
    ...
    const seconds = 0; // Los minutos no tienen segundos, pero mantenemos el formato
  ...
  const calcularMinutos = (horaMarcar, horaReferencia, tipo) => {
    if (!horaMarcar) return 0;
    const [h1, m1] = horaMarcar.split(":").map(Number);
    const [h2, m2] = horaReferencia.split(":").map(Number);
    
    const total1 = h1 * 60 + m1;
    const total2 = h2 * 60 + m2;

    if (tipo === "tardanza") return total1 > total2 ? total1 - total2 : 0;
    if (tipo === "extra") return total2 - total1; // Para entrada temprana
    if (tipo === "extra_salida") return total1 > total2 ? total1 - total2 : 0;
    return 0;
  };
  ...
  const calcularTiempoExtra = (entrada, salida) => {
    ...
    // Si llegó temprano, calcular extra entrada + extra salida
    const extraEntrada = calcularMinutos(entrada, "09:00:00", "extra");
    const extraSalida = calcularMinutos(salida, "18:00:00", "extra_salida");
  ...
  const minutosDebidos = useMemo(() => {
    // Según el HTML: mDebidos = mTotalTardanza (m.minT)
  ...
  // Según el HTML: mBalance = m.minE - m.minT
  const minutosDebidosExtras = tiempoExtraTotal - minutosTardanzaTotal;
                  ...
                  <div className="bg-gray-50 rounded-xl border border-gray-200 p-3 shadow-sm">
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="flex justify-between">
                        <span className="text-gray-900">Total Tardanza:</span>
                        <span className="font-semibold text-red-700">{formatMinutesToTime(minutosTardanzaTotal)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-900">Minutos Debidos:</span>
                        <span className="font-semibold text-green-700">{formatMinutesToTime(minutosDebidos)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-900">Tiempo Extra:</span>
                        <span className="font-semibold text-blue-700">{formatMinutesToTime(tiempoExtraTotal)}</span>
                      </div>
                      <div className="flex justify-between pt-2 border-t">
                        <span className="text-gray-900 font-medium">Balance Final:</span>
                        <span className={`font-bold ${minutosDebidosExtras >= 0 ? "text-green-700" : "text-red-700"}`}>
                          {minutosDebidosExtras >= 0 ? "+" : ""}
                          {formatMinutesToTime(Math.abs(minutosDebidosExtras))}
                        </span>
                      </div>
                    </div>
                  </div>
